...
| RZ-141 | Wire `RunState` into `Main.gd`/`MissionController.gd` | `game/scripts/Main.gd`, `game/scripts/mission/MissionController.gd`, `game/autoload/GameState.gd` | `Main.gd` creates a `RunState` via `RunState.new_run()` instead of just calling `GameState.start_new_run()`; `MissionController` sources its 3 squads from `RunState.commanders`/roster meta instead of hardcoding fresh ones, writes `unit_count`/gold back to `RunState` at mission end, and a mission's commander death calls `RunState.on_commander_died()` | RZ-054 ✅ | engine | Open |
