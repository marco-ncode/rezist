...
| RZ-090 | Save/Load wired to Main Menu "Continue" | `game/scripts/mission/MissionController.gd` (or wherever a mission-end/run-level save point is decided) | `MainMenuController`'s Continue button already calls `SaveManager.load()`/`has_save()` (RZ-074) — what's missing is a call to `SaveManager.save(GameState.run_state, MainMenuController.SAVE_SLOT)` at a sensible point in the mission flow (e.g. after each mission resolves), so Continue actually has something to load in practice | RZ-055 ✅, RZ-074 ✅ | engine | Open |
