...
|---|---|---|---|---|---|---|
| RZ-139 | Fix `UnitData.get_class()` name collision | `game/data_runtime/UnitData.gd`, and every call site (`game/core/squad/Squad.gd`, `game/scripts/mission/MissionController.gd`, `game/core/abilities/AbilityRegistry.gd` if applicable) | `get_class` renamed (e.g. `get_unit_class`) so it no longer shadows `Object.get_class()`; `godot --headless --path game --import` produces no "overrides a method from native class" error; `test_economy.gd`'s 3 currently-failing `Squad`-dependent assertions pass | RZ-044 ✅ | engine | **Open — top priority, see docs/HANDOFF.md "CI Findings"** |
| RZ-140 | Fix `staggered` type inference in `CombatResolver.gd` | `game/core/combat/CombatResolver.gd` line ~49 | Add explicit `: bool` type annotation to the `staggered` local; `--import` produces no "Cannot infer the type" error | RZ-045 ✅ | engine | **Open — top priority, see docs/HANDOFF.md "CI Findings"** |
...
| RZ-054 | Implement `core/run/RunState.gd` | `game/core/run/RunState.gd` | Tracks commander roster, gold, current difficulty; exposes `on_commander_died`, `is_run_over()`; unit test in `tests/test_run_state.gd` | RZ-046 ✅, RZ-051 ✅ | engine | Open |
| RZ-055 | Implement `core/run/SaveManager.gd` | `game/core/run/SaveManager.gd` | Serializes/deserializes `RunState` to `user://saves/<slot>.json` per `docs/DATA_SCHEMA.md` "Save File" schema; round-trip test | RZ-054 | engine | Open |
| RZ-135 | `tests/test_save_load_roundtrip.gd` | `tests/test_save_load_roundtrip.gd` | Save a populated `RunState`, load it, assert deep equality on every field | RZ-055 | tooling | Open |
...
| RZ-074 | Main Menu scene | `game/scenes/ui/MainMenu.tscn`, `game/scripts/ui/MainMenu.gd` | New Run starts `Mission.tscn` (or `CampaignMap.tscn` once RZ-082 lands) with a fresh seed; Continue loads a save if one exists; Quit exits | RZ-060 ✅ | ui | Open |
| RZ-075 | Mission Prep screen | `game/scenes/ui/MissionPrep.tscn`, `game/scripts/ui/MissionPrepController.gd`, small refactor of `MissionController._spawn_squads()` to accept player-chosen tiles instead of auto-placing | Player sees the generated grid before wave 1, drags/clicks each squad onto a deployment-zone tile, `[Start]` only enabled once every squad is placed | RZ-062 ✅ | ui | Open |
...
| RZ-085 | Armory/Upgrade screen | `game/scenes/ui/Armory.tscn`, `game/scripts/ui/ArmoryController.gd` | Spend gold via `Economy.upgrade_cost/ability_cost/relic_cost`; UI never shows a raw HP/damage number (ADR-0005) | RZ-051 ✅, RZ-067 ✅ | ui | Open |
| RZ-088 | Permadeath flow polish | `game/scripts/squad/*.gd` | Commander death removes squad from `RunState.commanders` immediately, plays a distinct audio/visual beat, cannot be undone even by reload of the same in-progress mission | RZ-054, RZ-046 ✅ | engine | Blocked on RZ-054 |
...
- `RZ-047` (Breach ability + AbilityRegistry), `RZ-049` (EnemyAI), `RZ-051` (Economy), `RZ-060/061/062` (Main scene + grid render + mission controller) are already implemented in this session — see `docs/HANDOFF.md` for exactly what's in and what's stubbed.
