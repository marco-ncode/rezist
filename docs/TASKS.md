...
| RZ-086 | Roster/Commander screen | `game/scenes/ui/Roster.tscn`, `game/scripts/ui/RosterController.gd` | Lists `RunState.commanders` (portrait placeholder, trait, relic, alive/dead) per UX_UI.md §7; fallen commanders shown in a separate "history" section, never removed from the list | RZ-054 ✅ | ui | Open |
| RZ-142 | Implement exposed-commander vulnerability (last-stand combat) | `game/core/squad/Squad.gd`, `game/core/squad/Commander.gd`, `game/scripts/mission/MissionController.gd` | Once a squad has 0 units and `commander_lost` fires, the commander becomes a targetable, vulnerable entity on the grid (duck-typed like `Unit`/`Enemy`: `get_position()`, `is_alive()`, `apply_damage()`, `to_combat_data()`), included in `MissionController`'s `all_units` passed to `EnemyAI`; taking lethal damage calls `Commander.die()`, which (now that RZ-141 sources squads from `RunState`) automatically reaches `RunState.on_commander_died()`. Today nothing ever damages a `Commander`, so permadeath cannot occur through normal play — this closes that gap | Must | M | RZ-046 ✅, RZ-049 ✅ | engine | Open |
