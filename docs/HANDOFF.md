...
**Last updated:** 2026-09-25, Session 1 (see `AGENTS.md`), continuing on the `dev` branch. Most recent landed work: RZ-142 (exposed-commander last-stand combat), RZ-074 (Main Menu scene), RZ-088 (permadeath UI feedback), RZ-090 (mission-end autosave, wired to Continue), RZ-143 (fixed two scripts silently unable to load in the real engine since the original bootstrap — see "CI / Build Status" below, point 4, before trusting any earlier green-CI claim in this file), RZ-089 (Run Summary screen on total wipe; also fixed a real gap this surfaced — the mission resolution panel had no Continue button at all, so the player was stuck there after every mission), RZ-085 (Armory screen — functional Class Tiers/Relics purchases, informational-only Abilities tab; new gap surfaced and tracked as RZ-144).
...
- **Roster screen** — doesn't exist yet (RZ-086). The Armory (RZ-085, see above) now covers spending gold, but there's still no dedicated place to see a commander's trait, equipped relic, and squad size at a glance, or the fallen-commander history (UX_UI.md §7).
...
1. **RZ-086 — Roster/Commander screen.** Spec exists (UX_UI.md §7), its only dependency (RZ-054) is done, and it's now more useful than ever: a player has no way to see who's in the roster, their trait/relic (now assignable via the Armory, RZ-085), or the fallen-commander history outside the Armory's per-commander tabs.
