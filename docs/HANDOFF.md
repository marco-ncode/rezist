...
**Last updated:** 2026-09-25, Session 1 (see `AGENTS.md`), continuing on the `dev` branch. Most recent landed work: RZ-142 (exposed-commander last-stand combat), RZ-074 (Main Menu scene), RZ-088 (permadeath UI feedback), RZ-090 (mission-end autosave, wired to Continue), RZ-143 (fixed two scripts silently unable to load in the real engine since the original bootstrap — see "CI / Build Status" below, point 4, before trusting any earlier green-CI claim in this file), RZ-089 (Run Summary screen on total wipe; also fixed a real gap this surfaced — the mission resolution panel had no Continue button at all, so the player was stuck there after every mission).
...
- **Armory/Upgrade, Roster screens** — none exist (RZ-085, RZ-086). There's a Main Menu (RZ-074), mission-end saving (RZ-090), and a Run Summary screen for a total wipe (RZ-089) now, but there's still no way to view the roster or spend gold outside a mission.
...
1. **RZ-086 — Roster/Commander screen.** Spec exists (UX_UI.md §7), its only dependency (RZ-054) is done, and it's now more useful than before RZ-090: a player picking Continue on the Main Menu has no way to see who's in the roster, their trait/relic, or the fallen-commander history before jumping into Mission Prep.
2. **RZ-085 — Armory/Upgrade screen.** Spec exists (UX_UI.md §6), deps (RZ-051, RZ-067) are done. There's now a full New Run → Mission → mission-end save → Continue loop (RZ-074/090) and a way for a run to actually end (RZ-089), but gold earned across missions still has nowhere to be spent.
3. Open the project in the actual Godot editor and playtest Main → Main Menu → Mission Prep → Mission → (repeat, including a full total-wipe run through to the new Run Summary screen) by eye (CI proves the code *compiles and the unit tests pass*, not that the flow *feels* right). In particular: verify the new Main Menu's difficulty sub-panel reads clearly, that Continue actually offers a save after playing one mission and loading it resumes the right roster/gold state (RZ-090), that the mission resolution panel's new Continue button correctly routes to the next Mission Prep or to Run Summary on a wipe (RZ-089), deployment-zone tiles are visually distinct enough, squad buttons showing real commander names read well, a squad's unit count visibly carrying over between missions is legible without a number (ADR-0005), that an exposed last-stand commander (RZ-142) reads clearly as "vulnerable" on screen (currently just a slightly larger amber square, same color as a normal commander marker), and that the RZ-088 permadeath toasts are readable/well-timed rather than flashing past too fast.
