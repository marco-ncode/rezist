...
**Last updated:** 2026-09-25, Session 1 (see `AGENTS.md`), continuing on the `dev` branch. Most recent landed work: RZ-142 (exposed-commander last-stand combat), RZ-074 (Main Menu scene), RZ-088 (permadeath UI feedback), RZ-090 (mission-end autosave, wired to Continue), RZ-143 (fixed two scripts silently unable to load in the real engine since the original bootstrap — see "CI / Build Status" below, point 4, before trusting any earlier green-CI claim in this file), RZ-089 (Run Summary screen on total wipe; also fixed a real gap this surfaced — the mission resolution panel had no Continue button at all, so the player was stuck there after every mission), RZ-085 (Armory screen — functional Class Tiers/Relics purchases, informational-only Abilities tab; new gap surfaced and tracked as RZ-144), RZ-086 (Roster screen — alive-commander cards + fallen-commander history).
...
- `[Start]` only enables once every squad is placed; hands the chosen tiles to `GameState.mission_deployment_positions` and transitions to `Mission.tscn`. If the roster is empty (total wipe — reachable mainly by loading a stale finished-run save), `_ready()` redirects straight to `RunSummary.tscn` (RZ-089) instead of building a Mission Prep UI with nothing to place
...
- Win (all waves cleared + battlefield clear) / lose (all squads wiped) both resolve to a simple HUD panel with gold payout, now with a **Continue** button (RZ-089 — previously missing entirely, so the player was stuck on that panel with no way to leave the scene after any mission, win or lose). Continue routes to `MissionPrep.tscn` for the next mission, or to the new `RunSummary.tscn` (below) if `RunState.is_run_over()` (every commander has fallen)
...
- **Every mission end now writes a save** (RZ-090): `_end_mission()` calls `SaveManager.save(GameState.run_state, SaveManager.DEFAULT_SLOT)` unconditionally, win or lose, right after roster metadata/gold are updated. This is the only save checkpoint that exists (no mid-mission saves, ADR-0007) and it's what makes the Main Menu's Continue button (RZ-074) actually usable in practice, not just correctly wired. Each mission's safehouse-saved count is also folded into `RunState.total_safehouses_saved` (RZ-089, `SAVE_VERSION` bumped to 2 — `from_save_dict()` defaults it to `0` for an older v1 save).
- 9 audio events fire through `AudioManager.play_event()` (no real SFX yet, but the pipeline is proven end-to-end)

**Run Summary screen** (`game/scenes/RunSummary.tscn`, `game/scripts/ui/RunSummary.gd`, RZ-089): reached from the mission resolution panel's Continue button once `RunState.is_run_over()` is true, or directly from Mission Prep's `_ready()` if it's ever loaded with an empty roster. Shows "RUN OVER" (UX_UI.md §8's alternate "CITY SECURED" headline for full campaign completion is never shown — `is_run_over()` only detects the wipe condition, since there's no `CampaignState` to detect the other, RZ-081), "Commanders lost: N" (counted from `RunState.commanders`), and "Safehouses saved (total): N" (`RunState.total_safehouses_saved`). "Reached: District N of ~11" from the wireframe is omitted — nothing to report with a single hardcoded district and no campaign graph. `[Start New Run]` returns to `MainMenu.tscn`.

**Armory screen** (`game/scenes/Armory.tscn`, `game/scripts/ui/Armory.gd`, RZ-085): reached from a new "Armory" button on Mission Prep's bottom bar (`[Back]` returns there — no Campaign Map to reach it from yet). Pick a commander from the roster strip, then one of three tabs: **Class Tiers** (real — spends `Economy.upgrade_cost()`, bumps the squad's level in `RunState`'s roster meta, which actually changes next mission's stats via `UnitData.get_level_stats()`), **Relics** (real — spends `Economy.relic_cost()`, sets `Commander.relic_id`, already round-tripped by `SaveManager`; the relic's actual combat *effect* is still the pre-existing RZ-109 gap, unaffected either way), **Abilities** (informational only — shows the class's ability name and `Economy.ability_cost()`, no purchase button, since nothing currently gates ability activation by level or purchase — see the gap below, tracked as RZ-144).

**Roster screen** (`game/scenes/Roster.tscn`, `game/scripts/ui/Roster.gd`, RZ-086): reached from a new "Roster" button on Mission Prep's bottom bar, alongside Armory's (`[Back]` returns there too). Lists every alive commander — name/class/level, trait (name + description, or "None"), relic (name, or "None"), squad size as dots only (never a "(5/6)" fraction like UX_UI.md §7's wireframe shows — ADR-0005 bans a number for squad strength everywhere else, this screen isn't an exception) — and, below a separator, every fallen commander by name (`RunState.commanders` keeps a dead entry forever, only its roster metadata is erased). The wireframe's "died Mission 4 (Brute)" annotation is omitted — nothing anywhere tracks which mission or enemy killed a commander.
...
- **No cause-of-death tracking for fallen commanders** — the Roster screen's fallen-commander history (RZ-086, see above) lists names only; UX_UI.md §7's "died Mission 4 (Brute)" annotation would need a mission counter and the killing enemy's type threaded through `Commander.die()` → `RunState.on_commander_died()`, neither of which exists. Not tracked under a specific RZ id yet — small enough to fold into whichever task next touches that signal chain.
- **Ability access isn't gated by level or purchase** — every class's ability (only Breach/Riot is actually implemented, RZ-048) works unconditionally from mission 1, regardless of squad level, contradicting GDD's "L2: class specialization unlocked, ability purchasable" intent. `Economy.ability_cost()` is only ever *displayed* (Armory's Abilities tab, RZ-085), never actually spent, since there's no persisted per-commander "unlocked" flag and no gate anywhere in `MissionController`/`AbilityRegistry` to check it against. Deliberately deferred rather than folded into RZ-085, since it would touch the same ability-activation code path RZ-143 found silently broken for months — not a change to make hastily. Tracked as RZ-144.
...
1. **RZ-048 — Implement Focused Volley + Line Charge abilities.** Must-priority, unblocks RZ-100/101 (Marksman/Barricade class data), and removes the benign-but-noisy `SCRIPT ERROR: DataLoader: ability 'focused_volley' declares effect...` assertion that's shown up in every CI run's log since the original bootstrap (see "CI / Build Status" below) — the vertical slice just never actually exercises it since only Riot squads spawn.
2. **RZ-144 — Wire ability-unlock enforcement.** `Economy.ability_cost()` is currently only *displayed* (Armory's Abilities tab), never spent — every class's ability works unconditionally regardless of level or purchase, which doesn't match GDD's "L2: ability purchasable" intent. Deliberately deferred out of RZ-085 since it touches the same ability-activation code path RZ-143 found silently broken for months; do this one carefully, with real test coverage for the new gating logic before it ships.
3. Open the project in the actual Godot editor and playtest Main → Main Menu → Mission Prep → (Armory / Roster) → Mission → (repeat, including a full total-wipe run through to the new Run Summary screen) by eye (CI proves the code *compiles and the unit tests pass*, not that the flow *feels* right). In particular: verify the new Main Menu's difficulty sub-panel reads clearly, that Continue actually offers a save after playing one mission and loading it resumes the right roster/gold state (RZ-090), that the mission resolution panel's new Continue button correctly routes to the next Mission Prep or to Run Summary on a wipe (RZ-089), that the Armory's tabs read clearly and a Class Tiers purchase actually changes the squad's stats next mission (RZ-085), that the Roster screen's commander cards are legible and the fallen-commander list reads clearly as permanent history (RZ-086), deployment-zone tiles are visually distinct enough, squad buttons showing real commander names read well, a squad's unit count visibly carrying over between missions is legible without a number (ADR-0005), that an exposed last-stand commander (RZ-142) reads clearly as "vulnerable" on screen (currently just a slightly larger amber square, same color as a normal commander marker), and that the RZ-088 permadeath toasts are readable/well-timed rather than flashing past too fast.
